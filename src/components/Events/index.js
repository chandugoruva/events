...
class Events extends Component {
  state = {
    eventStatus: constraintsStatus.initial,
    isClicked: false,
  }

  clickedEvent = status => {
    const {eventStatus} = this.state
    this.setState({eventStatus: status, isClicked: true})
    console.log(status)
  }

  render() {
    const {eventStatus, isClicked} = this.state
    const isClickedHeading = isClicked
      ? ''
    ...
    return (
      <div className="bg-color">
        ...
        <div className="events-status">
          <p className="events-heading">{isClickedHeading}</p>
          <ActiveEventRegistrationDetails eventDetails={eventStatus} />
        </div>
      </div>
    )
  }
